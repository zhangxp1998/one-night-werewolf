...
class LLMPlayer:
    ...
    def initialize_system_prompt(self, roles_db: dict):
        ...
        3. 夜晚行动时序（严格按照此顺序依次睁眼执行）：
           ① 狼人 (Werewolf) -> ② 爪牙 (Minion) -> ③ 守夜人 (Mason) -> ④ 预言家 (Seer) -> ⑤ 强盗 (Robber) -> ⑥ 捣蛋鬼 (Troublemaker) -> ⑦ 酒鬼 (Drunk)
           注：强盗换牌在前，捣蛋鬼换牌在后，酒鬼换牌最后。在白天推理逻辑时，请务必注意这种严格的时序先后关系！
        4. 白天游戏规则与流程：
           - 白天讨论共进行 3 轮完整发言。每一轮中，所有玩家（玩家1-6）依次轮流进行发言。
           - 发言中允许随着多轮次推进而进行撒谎、试探、伪装、透露真实信息或假跳身份。
           - 3 轮发言全部结束后，所有人根据发言记录进行秘密投票，票数最多的玩家被处决并揭晓胜负。
        5. 胜利条件：
        ...
        6. 你需要结合白天大家的发言，推理出自己当前的真实身份，并极力帮助自己的阵营（即你初始底牌所代表的阵营，除非你确信你被换了牌）获胜。
        7. 狼人要互相打配合、说谎或假跳身份。爪牙需要保护狼人、替狼人挡刀或吸引火力（爪牙死了但狼人没死，狼人赢）。村民需要通过逻辑排除法推理出真相。
